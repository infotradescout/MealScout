...
async function main(): Promise<void> {
  ...
  await runRole({
    ...
    checks: [
      ...
      {
        name: "owner owned fixture kitchen queue read",
        path: `/api/owner/kitchen-queue/${encodeURIComponent(env("SMOKE_OWNER_OWNED_FIXTURE_ID"))}`,
        expectedStatus: 200,
      },
      {
        name: "owner owned fixture order history read",
        path: `/api/owner/orders/${encodeURIComponent(env("SMOKE_OWNER_OWNED_FIXTURE_ID"))}`,
        expectedStatus: 200,
      },
      {
        name: "owner unowned fixture kitchen queue negative check",
        path: `/api/owner/kitchen-queue/${encodeURIComponent(env("SMOKE_OWNER_UNOWNED_FIXTURE_ID"))}`,
        expectedStatus: 403,
      },
      {
        name: "owner unowned fixture order history negative check",
        path: `/api/owner/orders/${encodeURIComponent(env("SMOKE_OWNER_UNOWNED_FIXTURE_ID"))}`,
        expectedStatus: 403,
      },
    ],
  });
