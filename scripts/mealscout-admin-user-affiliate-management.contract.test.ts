...
const affiliateAdminRoutes = readFileSync(
  "server/routes/admin/affiliateAdminRoutes.ts",
  "utf8",
);
...
for (const snippet of requiredDashboardSnippets) {
  if (!dashboard.includes(snippet)) {
    throw new Error(
      `Missing admin user affiliate management snippet: ${snippet}`,
    );
  }
}

const managementSectionIndex = dashboard.indexOf("Affiliate Management");
const affiliateSectionIndex = dashboard.indexOf(
  "Affiliate Link",
  managementSectionIndex,
);
const affiliateCopyIndex = dashboard.indexOf(
  "Copy Link",
  affiliateSectionIndex,
);
if (affiliateSectionIndex === -1 || affiliateCopyIndex === -1) {
  throw new Error(
    "Admin user card must contain Affiliate Link section with Copy Link",
  );
}
if (managementSectionIndex === -1 || affiliateSectionIndex === -1) {
  throw new Error(
    "Affiliate management section must exist inside the admin user card",
  );
}
...
const clipboardCalls = [
  ...dashboard.matchAll(/navigator\.clipboard\.writeText\(([^)]*)\)/g),
];
for (const match of clipboardCalls) {
  ...
  if (
    ...
    throw new Error(
      "User-card copy/share handlers must not copy admin dashboard focus URLs",
    );
  }
}

const copyLinkHandlerStart = dashboard.lastIndexOf(
  "onClick={async () =>",
  affiliateCopyIndex,
);
const copyLinkHandler = dashboard.slice(
  copyLinkHandlerStart,
  affiliateCopyIndex + 200,
);
if (
  copyLinkHandler.includes("/admin/dashboard") ||
  !copyLinkHandler.includes("affiliateLink")
) {
  throw new Error(
    "Primary Copy Link must copy the public affiliate link, not admin dashboard URL",
  );
}
if (
...
const openLinkHandlerStart = dashboard.lastIndexOf(
  "onClick={() =>",
  dashboard.indexOf("Open Link", affiliateSectionIndex),
);
const openLinkHandler = dashboard.slice(
  openLinkHandlerStart,
  dashboard.indexOf("Open Link", affiliateSectionIndex) + 200,
);
if (
  openLinkHandler.includes("/admin/dashboard") ||
  !openLinkHandler.includes("affiliateLink")
) {
  throw new Error(
    "Open Link must open the public affiliate link, not admin dashboard URL",
  );
}
...
const affiliateDisplaySlice = dashboard.slice(
  affiliateSectionIndex,
  dashboard.indexOf("Internal token", affiliateSectionIndex),
);
if (
  ...
  throw new Error(
    "Primary Affiliate Link display/copy/open must not use admin paths",
  );
}

const notApplicableIndex = dashboard.indexOf(
  "Not applicable for internal admin accounts.",
);
const affiliateLinkConstructionIndex = dashboard.indexOf(
  "const affiliateLink = buildCanonicalAffiliateLink",
  notApplicableIndex,
);
if (
  notApplicableIndex === -1 ||
  affiliateLinkConstructionIndex === -1 ||
  notApplicableIndex > affiliateLinkConstructionIndex
) {
  throw new Error(
    "Internal admin not-applicable check must happen before affiliate link construction",
  );
}

const notApplicableSlice = dashboard.slice(
  notApplicableIndex,
  affiliateLinkConstructionIndex,
);
if (
  notApplicableSlice.includes("Copy Link") ||
  notApplicableSlice.includes("Open Link")
) {
  throw new Error(
    "Internal admin accounts must not render affiliate link controls",
  );
}

const settingsEndpointIndex = dashboard.indexOf(
  "`/api/admin/affiliates/users/${payload.userId}`",
);
const settingsButtonIndex = dashboard.indexOf(
  "Save Affiliate Settings",
  affiliateSectionIndex,
);
if (settingsEndpointIndex === -1 || settingsButtonIndex === -1) {
  throw new Error(
    "Supported single-user affiliate settings must be editable from the admin user card",
  );
}
...
for (const snippet of affiliatePageReportingSnippets) {
  if (!affiliatePage.includes(snippet)) {
    throw new Error(
      `Aggregate affiliate page must remain a reporting/overview surface: ${snippet}`,
    );
  }
}
...
if (!affiliatePage.includes("/api/admin/affiliates/users")) {
  throw new Error(
    "Affiliate page may remain as aggregate affiliate user overview",
  );
}
...
for (const snippet of forbiddenDashboardSnippets) {
  if (dashboard.includes(snippet)) {
    throw new Error(
      `Admin user card must not include fake/unsupported affiliate control: ${snippet}`,
    );
  }
}
...
for (const snippet of forbiddenMutationSnippets) {
  if (dashboardAffiliateSlice.includes(snippet)) {
    throw new Error(
      `Admin dashboard affiliate link display must not touch payout logic: ${snippet}`,
    );
  }
}
...
if (
  affiliateAdminRoutes.includes("affiliateTag") &&
  !affiliateAdminRoutes.includes("affiliatePercent")
) {
  throw new Error("Existing admin affiliate route shape changed unexpectedly");
}

if (
  dashboard.includes('return "business_owner"') ||
  dashboard.includes('"business_owner"')
) {
...
if (customerNotRequiredIndex === -1 || invalidMissingIndex === -1) {
  throw new Error(
    "Customer users must resolve before invalid_missing_business",
  );
}
