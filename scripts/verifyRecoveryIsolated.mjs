...
async function run(name, command, args, extraEnv = {}) {
  ...
  try {
    await new Promise((done, reject) => {
      const child = spawn(command, args, { cwd: root, env: { ...env, ...extraEnv }, stdio: "inherit", timeout: 1_800_000 });
...
try {
  ...
  await run("pinned-native-postgres16-tooling", "npm", ["install", "--prefix", tooling, "--no-save", "--package-lock=false", "--no-audit", "--no-fund", "@embedded-postgres/linux-x64@16.14.0-beta.17"]);
  const nativeBin = join(tooling, "node_modules/@embedded-postgres/linux-x64/native/bin");
