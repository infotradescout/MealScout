...
if (customerNotRequiredIndex === -1 || invalidMissingIndex === -1) {
  throw new Error("Customer/non-business users must resolve before invalid_missing_business");
}

const attachmentBadgeIndex = dashboard.indexOf(
  "{isBusinessBearingUserType(user.userType) && (",
);
const attachmentTextIndex = dashboard.indexOf("attachment:", attachmentBadgeIndex);
if (attachmentBadgeIndex === -1 || attachmentTextIndex === -1) {
  throw new Error("Business attachment badge must be gated by business-bearing role");
}
...
if (subscriptionControlIndex === -1 || subscriptionGateIndex === -1) {
  throw new Error("Send Monthly Link must be gated by explicit business subscription roles");
}
...
for (const snippet of forbiddenDashboardSnippets) {
  if (dashboard.includes(snippet)) {
    throw new Error(`Admin dashboard contains forbidden stale/invented display snippet: ${snippet}`);
  }
}
...
for (const value of dropdownOptionMatches) {
  if (value.includes("_owner") || canonicalRoles.includes(value) || value === "unknown") {
    if (!dropdownAllowedValues.has(value) && ![
      "food_truck_owner",
      "bar_owner",
      ...
      throw new Error(`Role dropdown contains unsupported userType value: ${value}`);
    }
  }
}

const businessRedirectStart = useAuth.indexOf("const setupOnlyRoutes =");
const businessRedirectEnd = useAuth.indexOf("if (!setupOnlyRoutes) return;", businessRedirectStart);
if (businessRedirectStart === -1 || businessRedirectEnd === -1) {
  throw new Error("useAuth business onboarding redirect block must be discoverable");
}
const businessRedirectBlock = useAuth.slice(businessRedirectStart, businessRedirectEnd);
if (businessRedirectBlock.includes("pathname.startsWith(\"/parking-pass\")")) {
  throw new Error("Parking Pass management must not be blocked by unrelated business onboarding redirect");
}
...
for (const phrase of forbiddenFeaturePhrases) {
  if (audit.toLowerCase().includes(phrase)) {
    throw new Error(`Admin truth audit must not introduce feature/design scope: ${phrase}`);
  }
}
