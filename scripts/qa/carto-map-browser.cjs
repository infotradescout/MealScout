...
const workerNames=fs.readdirSync(assets).filter(file=>/^maplibre-gl-worker-.*\.js$/.test(file));
assert.equal(workerNames.length,1,'Production MapLibre worker must be emitted by Vite');
const workerName=workerNames[0];
const workerBridge=path.join(evidence,'carto-worker-url.fixture.js');
fs.writeFileSync(workerBridge,'export default '+JSON.stringify('/assets/'+workerName)+';');
...
function bundle(key){
 ...
 return buildSync({stdin:{contents:source,loader:'tsx',resolveDir:root},absWorkingDir:root,bundle:true,write:false,outfile:'map-fixture.js',platform:'browser',format:'iife',jsx:'automatic',alias:{'maplibre-gl/dist/maplibre-gl-worker.mjs?worker&url':workerBridge},define:{'process.env.NODE_ENV':'"production"','import.meta.env.VITE_CARTO_BASEMAPS_API_KEY':JSON.stringify(key)}}).outputFiles.find(f=>f.path.endsWith('.js')).text;
}
...
async function main(){
 ...
 const security=await require('./maplibre-security.browser.cjs').run();if(security.fail)process.exitCode=1;
}
