...
`npm run gate:release` runs that same sequence, then enables strict production configuration validation and read-only live probes. It must be run with the intended production environment values before a production promotion. The live stage performs GET requests only.
