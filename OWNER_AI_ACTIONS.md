...
MealScout Owner AI Actions is a model-neutral owner workflow. An owner can use an AI or automation tool they already have, free or paid, to prepare a complete business update. The same JSON packet can be submitted by an HTTP-capable tool or pasted into MealScout's `/owner-ai` workspace.

The AI is never the approver. A connector key can only read the business attached to that key and create inert drafts. Only the session-authenticated actual restaurant owner can edit, cancel, or approve a draft.

## Safety and execution order

1. A connector reads its owner-scoped context and the current version fingerprints.
2. It creates a draft using an `Idempotency-Key`. Nothing is mutated, hosted, queued, or published.
3. MealScout creates deterministic Facebook, Instagram, and X copy plus branded SVG previews. When the AI supplies platform-specific copy or an image, that exact selected alternative becomes the approval preview.
4. The owner opens the returned `/owner-ai?restaurantId=...&ownerAiDraft=...` URL and reviews the packet, normalized plan, social copy, previews, and any media-rights affirmation.
5. Approval rechecks the actual owner, draft revision, expiry, and every canonical version fingerprint.
6. Approved remote canonical media is validated against private-network/SSRF targets and re-hosted when Cloudinary is available.
7. One database transaction applies canonical MealScout changes with upsert/archive semantics and creates durable social intents.
8. After commit, MealScout rasterizes generated cards to public PNG and processes Facebook, Instagram, then X.
9. Each platform reports `posted`, `failed`, or `manual_required`. Social failure never rolls back the committed MealScout update.
...
Create, list, and revoke connector credentials while signed in:
...
Connector scopes are deliberately limited to:

- `owner_ai:context`
- `owner_ai:drafts:create`
- `owner_ai:drafts:read`

There is no connector route or scope for update, cancel, approve, apply, media hosting, queueing, or publishing.
...
The status route returns safe canonical and per-platform outcomes only when the draft was created by that exact connector key for that exact owner/business attachment. It does not grant edit, cancel, approval, apply, or publishing authority. This lets the same AI chat report what happened after the owner approves in MealScout.

For a free AI without HTTP tools, the owner workspace's **Copy current context for any AI** button packages the same current facts, version fingerprints, contract links, and output shape for copy/paste. The paste box accepts either the canonical `{ "expectedVersions": ..., "packet": ... }` request or the inner packet shorthand.
...
Run `migrations/123_owner_ai_action_drafts.sql` before enabling these routes against a database. Generated social images require configured Cloudinary credentials. Without image hosting, the canonical MealScout transaction can still succeed when its own content does not require a hosted deal image, while affected social platforms report `manual_required` and publish nothing.
