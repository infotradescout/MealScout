import { db } from "./db";
import { affiliateLinks, referrals, users } from "@shared/schema";
...
export async function resolveAffiliateUserId(ref: string): Promise<string | null> {
  if (!ref) return null;
  const normalizedRef = normalizeAffiliateTag(ref);
  ...
  const [user] = await db
    .select({ id: users.id })
    .from(users)
    .where(eq(users.affiliateTag, normalizedRef))
    .limit(1);
  ...
  const [referral] = await db
    .select({ affiliateUserId: referrals.affiliateUserId })
    .from(referrals)
    .where(eq(referrals.id, ref))
    .limit(1);
  return referral?.affiliateUserId || null;
}
