...
export class DatabaseStorage implements IStorage {
  ...
  private async selectHostsSafe(
    ...
    const select = [
      ...
      `${has("description") ? `${q("description")} as "description"` : `null as "description"`}`,
      `${has("google_place_id") ? `${q("google_place_id")} as "googlePlaceId"` : `null as "googlePlaceId"`}`,
      `${has("google_rating") ? `${q("google_rating")} as "googleRating"` : `null as "googleRating"`}`,
      `${has("google_review_count") ? `${q("google_review_count")} as "googleReviewCount"` : `null as "googleReviewCount"`}`,
      `${has("google_price_level") ? `${q("google_price_level")} as "googlePriceLevel"` : `null as "googlePriceLevel"`}`,
      `${has("google_business_status") ? `${q("google_business_status")} as "googleBusinessStatus"` : `null as "googleBusinessStatus"`}`,
      `${has("google_photos") ? `${q("google_photos")} as "googlePhotos"` : `null as "googlePhotos"`}`,
      `${has("google_categories") ? `${q("google_categories")} as "googleCategories"` : `null as "googleCategories"`}`,
      `${has("google_formatted_phone") ? `${q("google_formatted_phone")} as "googleFormattedPhone"` : `null as "googleFormattedPhone"`}`,
      `${has("business_hours") ? `${q("business_hours")} as "businessHours"` : `null as "businessHours"`}`,
      `${has("business_website") ? `${q("business_website")} as "businessWebsite"` : `null as "businessWebsite"`}`,
      `${has("menu_url") ? `${q("menu_url")} as "menuUrl"` : `null as "menuUrl"`}`,
      `${has("profile_source") ? `${q("profile_source")} as "profileSource"` : `null as "profileSource"`}`,
      `${has("profile_last_synced") ? `${q("profile_last_synced")} as "profileLastSynced"` : `null as "profileLastSynced"`}`,
      `${has("facebook_page_id") ? `${q("facebook_page_id")} as "facebookPageId"` : `null as "facebookPageId"`}`,
      `${has("facebook_page_url") ? `${q("facebook_page_url")} as "facebookPageUrl"` : `null as "facebookPageUrl"`}`,
      `${has("created_at") ? `${q("created_at")} as "createdAt"` : `null as "createdAt"`}`,
      `${has("updated_at") ? `${q("updated_at")} as "updatedAt"` : `null as "updatedAt"`}`,
    ];
  ...
  async createTruckManualSchedule(
    ...
    return this.truckLiveOpsRepository.createTruckManualSchedule(schedule);
  }

  async getTruckManualSchedules(
    truckId: string,
  ): Promise<TruckManualSchedule[]> {
    return this.truckLiveOpsRepository.getTruckManualSchedules(truckId);
  }
  ...
  async deleteTruckManualSchedule(
    ...
    return this.truckLiveOpsRepository.deleteTruckManualSchedule(
      scheduleId,
      truckId,
    );
  }
  ...
  async createTruckParkingReport(
    ...
    return this.truckLiveOpsRepository.createTruckParkingReport(report);
  }
  ...
  async getTruckParkingReports(
    ...
    return this.truckLiveOpsRepository.getTruckParkingReports(truckId, options);
  }
  ...
  async startTruckSession(
    ...
    return this.truckLiveOpsRepository.startTruckSession(
      restaurantId,
      deviceId,
      userId,
    );
  ...
  async endTruckSession(restaurantId: string, userId: string): Promise<void> {
    return this.truckLiveOpsRepository.endTruckSession(restaurantId, userId);
  }

  async getActiveTruckSession(
    restaurantId: string,
  ): Promise<FoodTruckSession | undefined> {
    return this.truckLiveOpsRepository.getActiveTruckSession(restaurantId);
  }
  ...
  async hasRecentLocationUpdate(
    ...
    return this.truckLiveOpsRepository.hasRecentLocationUpdate(
      restaurantId,
      lat,
      lng,
      timeWindowMs,
  ...
  async upsertLiveLocation(
    ...
    return this.truckLiveOpsRepository.upsertLiveLocation(location);
  }
  ...
  async getLiveTrucksNearby(
    ...
    return this.truckLiveOpsRepository.getLiveTrucksNearby(
      lat,
      lng,
      radiusKm,
    );
  ...
  async getTruckLocationHistory(
    ...
    return this.truckLiveOpsRepository.getTruckLocationHistory(
      restaurantId,
      dateRange,
    );
  }
