  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (inserted.rows.length === 0) {
        ...
        if (!(new Date(row.expires_at).getTime() > Date.now())) return unresolved();
        ...
        if (row.state === "processing" && new Date(row.locked_until).getTime() > Date.now()) {
      ...
      void (async () => {
        try {
          const saved = await db.execute(sql`
            UPDATE idempotency_keys SET state = 'completed', status_code = ${status},
