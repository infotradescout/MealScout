          ...
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredJobs.map((job) => (
              <Link key={job.id} href={job.publicUrl as any}>
                <Card className="group h-full cursor-pointer border-[color:var(--border-subtle)] bg-[var(--bg-card)] shadow-clean transition hover:-translate-y-0.5 hover:border-amber-500/60 hover:shadow-clean-lg">
                  <CardContent className="flex h-full flex-col gap-4 p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-amber-500 text-black">
                        {job.roleType === "cook" || job.roleType === "prep" ? (
