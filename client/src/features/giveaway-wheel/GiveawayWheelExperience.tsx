...
export type GiveawayWheelBackgroundConfig = {
  imageUrl?: string;
  noiseUrl?: string;
};

export type GiveawayWheelLogoConfig = {
  src?: string;
  alt?: string;
};

export type GiveawayWheelThemeConfig = {
  stageGold?: string;
  stageRed?: string;
  stageTeal?: string;
  stageInk?: string;
...
export type GiveawayWheelConfig = {
  productName?: string;
  defaultTitle?: string;
  defaultEntries?: string[];
  background?: GiveawayWheelBackgroundConfig;
  logo?: GiveawayWheelLogoConfig;
  theme?: GiveawayWheelThemeConfig;
};

export type GiveawayWheelWinnerContext = {
  entries: WheelEntry[];
  winnerNumber: number;
};

export type GiveawayWheelExperienceProps = {
  config?: GiveawayWheelConfig;
  storageKey?: string | null;
  backHref?: string;
  backLabel?: string;
  onWinner?: (winner: WheelEntry, context: GiveawayWheelWinnerContext) => void;
};

export const GIVEAWAY_WHEEL_MODULE_VERSION = "0.1.0";
...
export const defaultGiveawayEntries = [
  "Food Truck Fan 01",
  "Food Truck Fan 02",
  "Food Truck Fan 03",
  "Food Truck Fan 04",
...
export const parseGiveawayEntries = (value: string): WheelEntry[] => {
...
const spinFullSpeedPhase = 0.48;
const spinFullSpeedDistance = 0.74;
const spinBaseDurationMs = 9800;
const spinDurationJitterMs = 1200;

const spinMotionProgress = (progress: number) => {
  if (progress <= spinFullSpeedPhase) {
    return (progress / spinFullSpeedPhase) * spinFullSpeedDistance;
  }

  const slowProgress =
    (progress - spinFullSpeedPhase) / (1 - spinFullSpeedPhase);
  return (
    spinFullSpeedDistance +
    (1 - spinFullSpeedDistance) * easeOutCubic(slowProgress)
  );
};
...
const cssUrlValue = (url?: string) =>
  url ? `url("${url.replace(/"/g, '\\"')}")` : undefined;

export function GiveawayWheelExperience({
  config = {},
  storageKey = defaultStorageKey,
  backHref,
  backLabel = "Back",
  onWinner,
}: GiveawayWheelExperienceProps) {
  const defaultTitle = config.defaultTitle || config.productName || "MealScout Giveaway";
  const defaultEntries = config.defaultEntries?.length
    ? config.defaultEntries
    : defaultGiveawayEntries;
  const logo = config.logo || {};
  const palette = config.theme?.wheelColors?.length
    ? config.theme.wheelColors
    : defaultWheelColors;
  const [title, setTitle] = useState(defaultTitle);
  const [rawList, setRawList] = useState(defaultEntries.join("\n"));
  ...
  }, [rawList, soundOn, storageKey, title]);
  ...
  const entries = useMemo(() => parseGiveawayEntries(rawList), [rawList]);
  const fallbackWheelEntries = useMemo(
    () => parseGiveawayEntries(defaultEntries.join("\n")),
    [defaultEntries],
  );
  ...
  const spin = () => {
    ...
    const targetRotation = startRotation + 360 * (11 + randomIndex(5)) + delta;
    const duration = spinBaseDurationMs + randomIndex(spinDurationJitterMs);
    ...
    const animate = (timestamp: number) => {
      ...
      const pickedWinner = displayEntries[selectedIndex];
      setRotation(targetRotation);
      setWinner(pickedWinner);
      setIsSpinning(false);
      setShowFinale(true);
      onWinner?.(pickedWinner, {
        entries: displayEntries,
        winnerNumber: selectedIndex + 1,
      });
  ...
  const rootStyle = {
    "--ms-stage-gold": config.theme?.stageGold || "#f59e0b",
    "--ms-stage-red": config.theme?.stageRed || "#ef4444",
    "--ms-stage-teal": config.theme?.stageTeal || "#14b8a6",
    "--ms-stage-ink": config.theme?.stageInk || "#070605",
    "--ms-giveaway-bg-image": cssUrlValue(
      config.background?.imageUrl || defaultBackgroundImageUrl,
    ),
    "--ms-giveaway-noise-image": cssUrlValue(
...
export default GiveawayWheelExperience;
