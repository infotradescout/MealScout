...
- `/map` - compatibility entry. `client/src/App.tsx` routes this to `RedirectToScout`; Scout owns the public map.
...
| `/scout-prototype` | `RedirectToScout` in `client/src/App.tsx` | Compatibility redirect; the prototype implementation has been retired. |
| `/map` | `RedirectToScout` in `client/src/App.tsx`, then `client/src/pages/explore-preview-v2.tsx` | Scout owns the public Google map; there is no standalone map implementation. |
...
- The old `explore-preview.tsx`, `scout-prototype.tsx`, `map.tsx`, and `trending.tsx` page implementations were removed after their routes were verified against `client/src/App.tsx` and internal navigation references.
- `/scout-prototype`, `/map`, and `/trending` remain compatibility redirects to `/scout`.
- Scout and Parking Pass use the shared Google map implementation. The themed-tile, SVG-street, and Leaflet runtime fallbacks were removed so tests cannot validate an implementation users never receive.
